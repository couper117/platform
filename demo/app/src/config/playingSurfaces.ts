...
export type Surface = {
  ...
  positions?: string[];
  /**
   * Shapes a coach may pick, where the sport expresses one as a number string.
   * Football and rugby do; basketball and volleyball do not — five on court is
   * five on court, and offering a basketball coach "4-3-3" is the same mistake
   * as drawing them a football pitch.
   */
  formations?: string[];
...
const FOOTBALL: Surface = {
  label: 'Pitch', starters: 11, rows: [1, 4, 3, 3], opposed: true,
  formations: ['4-3-3', '4-4-2', '4-2-3-1', '3-5-2', '3-4-3', '5-3-2', '4-1-4-1'], band: [10, 46], tone: 'grass',
  markings: [
...
const RUGBY: Surface = {
  label: 'Pitch', starters: 15, rows: [3, 2, 3, 2, 3, 2], opposed: true,
  formations: ['3-2-3-2-3-2'], band: [10, 48], tone: 'grass',
  markings: [
    { t: 'rect', x: 2, y: 2, w: 96, h: 146 },
    { t: 'line', x1: 2, y1: 75, x2: 98, y2: 75 },
...
/**
 * The surface for a sport, or null when there is none defined.
 *
 * Deliberately no fallback. Falling back to a pitch for any unrecognised TEAM
 * sport is how this went wrong in the first place — and it is not hypothetical:
 * chess is typed TEAM, and a chessboard is not a football pitch. A sport with no
 * entry here shows its rosters until somebody defines its surface, which is the
 * honest answer rather than a confident wrong one.
 */
export const surfaceFor = (sport?: { slug?: string; type?: string } | null): Surface | null =>
  (sport?.slug ? SURFACES[sport.slug] ?? null : null);

/** What to show instead, when there is no surface. */
export const NO_SURFACE_REASON: Record<string, string> = {
  TEAM: 'The line-ups for this sport are listed below.',
