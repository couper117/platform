...
const TeamLineupsPage = () => {
  ...
  const submit = () => {
    setError('');
    if (starters === 0) return setError(t('teamlineups.err_starter'));
    if (captain && roles[captain] === 'OUT') return setError(t('teamlineups.err_captain'));
    saveMutation.mutate();
  };
  ...
  if (!team) return <p className="opacity-50 py-10">{t('teamlineups.no_team')}</p>;

  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="space-y-2">
        <h1 className="text-4xl font-display uppercase tracking-tighter flex items-center gap-3">
          <ClipboardList className="text-red" /> {t('teamlineups.title_a')} <span className="text-red">{t('teamlineups.title_b')}</span>
        </h1>
        <p className="text-[10px] uppercase font-bold tracking-[0.4em] opacity-40">{t('teamlineups.subtitle')}</p>
      </div>

      {fxLoading ? (
        <Skeleton type="card" count={3} />
      ) : !fixtures?.length ? (
        <p className="opacity-50 py-10">{t('teamlineups.no_fixtures')}</p>
      ) : (
          ...
          {fixtures.map((f) => {
            ...
            return (
              <div key={f.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-white dark:bg-white/5 border border-surface-3 dark:border-white/10 rounded-2xl p-4 sm:p-5">
                ...
                >
                  {locked ? <><Lock size={14} /> {t('teamlineups.locked')}</> : <><Users size={14} /> {t('teamlineups.manage_lineup')}</>}
                </button>
              </div>
            );
          })}
      ...
      <AdminModal isOpen={!!editing} onClose={() => setEditing(null)} title={editing ? t('teamlineups.lineup_vs', { opp: opponentOf(editing)?.name || '' }) : ''}>
        <div className="space-y-5">
          ...
          <div className="flex items-center justify-between text-[10px] uppercase font-bold tracking-widest opacity-60">
            <span>{t('teamlineups.squad_hint')}</span>
            <span className="text-red">{t('teamlineups.starters', { count: starters })}</span>
          </div>
          ...
          <div className="max-h-72 overflow-y-auto space-y-2 pr-1">
              ...
              return (
                <div key={p.id} className="flex items-center justify-between bg-surface-2 dark:bg-white/5 rounded-xl px-3 py-2">
                  <button onClick={() => cycleRole(p.id)} className="flex items-center gap-2.5 flex-1 min-w-0 text-left">
                    <span className={`text-[8px] font-bold px-2 py-1 rounded uppercase w-14 text-center shrink-0 ${badge}`}>{t(`teamlineups.role_${role.toLowerCase()}`)}</span>
            ...
            {!roster.length && <p className="opacity-40 text-xs py-4">{t('teamlineups.no_players')}</p>}
          </div>
          ...
          <button onClick={submit} disabled={saveMutation.isPending} className="w-full bg-red text-white font-display text-lg uppercase tracking-widest py-3.5 rounded-xl hover:bg-red-dark transition-all flex items-center justify-center disabled:opacity-50">
            {saveMutation.isPending ? <Loader2 className="animate-spin" /> : <><Shield size={18} className="mr-2" /> {t('teamlineups.publish_lineup')}</>}
          </button>
        </div>
      </AdminModal>
    </div>
