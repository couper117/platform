...
import {
  Users, UserSquare2, CalendarDays, CheckCircle2, Clock, Target, ArrowRight,
  PlusCircle, ClipboardCheck, FileText, Newspaper, ChevronDown, Radio,
} from 'lucide-react';
...
/**
 * LEAGUE ADMIN DASHBOARD — running ONE competition (Rwanda Premier League here).
 *
 * The operational level: fixtures, match reports, standings, top scorers — all
 * scoped to the admin's assigned league (leagueId derived from the token in the
 * real app). Standings/top-scorers are computed from approved match data, so the
 * admin never re-enters numbers. Quick actions include Create Fixture but the
 * live scoring belongs to the assigned Match Reporter.
 */

const LEAGUE_ID = 1; // the admin's assigned league (from the token in production)
...
const QUICK = [
  { label: 'Create Fixture', sub: 'Add new match', icon: PlusCircle, to: '/admin/league/fixtures' },
  { label: 'Assign Reporter', sub: 'Assign to a match', icon: Radio, to: '/admin/league/reporters' },
  { label: 'Review Reports', sub: 'Pending reports (3)', icon: ClipboardCheck, to: '/admin/league/match-reports', badge: 3 },
  { label: 'Manage Teams', sub: 'Participating teams', icon: Users, to: '/admin/teams' },
  { label: 'Manage Players', sub: 'League players', icon: UserSquare2, to: '/admin/players' },
...
const LeagueDashboard = () => {
  ...
  const { data: leagueRes } = useQuery({ queryKey: ['la-league'], queryFn: () => getLeague(LEAGUE_ID) });
  const { data: fixturesRes } = useQuery({ queryKey: ['la-fixtures'], queryFn: () => getFixtures({ leagueId: LEAGUE_ID }) });
  const { data: teamsRes } = useQuery({ queryKey: ['la-teams'], queryFn: async () => (await apiClient.get('/teams', { params: { sportId: 1 } })).data });
  ...
  const cards = [
    { icon: Users, value: teams.length || standings.length, label: 'Teams', tint: '#16a34a' },
    { icon: UserSquare2, value: players, label: 'Players', tint: '#2563eb' },
    { icon: CalendarDays, value: fixtures.length, label: 'Fixtures', tint: '#7c3aed' },
    { icon: CheckCircle2, value: completed.length, label: 'Completed', tint: '#16a34a' },
    { icon: Clock, value: upcoming.length, label: 'Upcoming', tint: '#f5b301' },
    { icon: Target, value: goals, label: 'Goals', tint: '#dc2626' },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="font-display text-2xl uppercase tracking-tight text-primary sm:text-3xl">Welcome back, {user?.fullName?.split(' ')[0] || 'John'}! <span aria-hidden="true">👋</span></h1>
          <p className="mt-1 text-sm"><span className="font-semibold text-brand-text">{league.name || 'Rwanda Premier League'}</span> <span className="text-tertiary">· Season {league.season || '2025/2026'}</span></p>
        </div>
        <button className="inline-flex w-fit items-center gap-2 rounded-xl border border-hairline bg-surface px-3 py-2 text-sm font-semibold text-secondary">
          <CalendarDays size={15} /> Season {league.season || '2025/2026'} <ChevronDown size={14} className="opacity-50" />
        </button>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_320px_320px]">
        <div className="rounded-2xl border border-hairline bg-surface p-5">
          <div className="mb-3"><h2 className="font-display text-lg uppercase tracking-tight text-primary">League Overview</h2><p className="text-xs text-tertiary">Goals scored over the season</p></div>
          <div className="h-52">
        ...
        {/* Upcoming fixtures */}
        <div className="rounded-2xl border border-hairline bg-surface p-5">
          <div className="mb-3 flex items-center justify-between"><h2 className="font-display text-lg uppercase tracking-tight text-primary">Upcoming Fixtures</h2><Link to="/admin/fixtures" className="text-[11px] font-bold uppercase tracking-wider text-brand-text">View all</Link></div>
            ...
            {upcoming.slice(0, 3).map((f, i) => (
              <Link key={f.id} to={`/matches/${f.id}`} className="block border-b border-hairline/60 pb-3 last:border-0 last:pb-0">
                <div className="mb-1 flex items-center justify-between">
                  ...
                  <span className={`rounded-md px-1.5 py-0.5 text-[9px] font-bold uppercase ${i % 2 === 0 ? 'bg-brand/10 text-brand-text' : 'bg-gold/10 text-gold'}`}>{i % 2 === 0 ? 'Assigned' : 'Pending'}</span>
                </div>
        ...
        {/* Recent results */}
        <div className="rounded-2xl border border-hairline bg-surface p-5">
          <div className="mb-3 flex items-center justify-between"><h2 className="font-display text-lg uppercase tracking-tight text-primary">Recent Results</h2><Link to="/results" className="text-[11px] font-bold uppercase tracking-wider text-brand-text">View all</Link></div>
          <div className="space-y-3">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-hairline bg-surface p-5">
          <div className="mb-4 flex items-center justify-between"><h2 className="font-display text-lg uppercase tracking-tight text-primary">League Standings</h2><Link to={`/leagues/${LEAGUE_ID}`} className="text-[11px] font-bold uppercase tracking-wider text-brand-text">Full table</Link></div>
          <div className="overflow-x-auto scrollbar-hide">
            <table className="w-full min-w-[520px] text-left">
              <thead><tr className="text-[9px] font-bold uppercase tracking-widest text-tertiary"><th className="pb-2 pr-2">#</th><th className="pb-2">Team</th><th className="pb-2 px-1 text-center">P</th><th className="pb-2 px-1 text-center">W</th><th className="pb-2 px-1 text-center">D</th><th className="pb-2 px-1 text-center">L</th><th className="pb-2 px-1 text-center">GD</th><th className="pb-2 px-1 text-center">Pts</th><th className="pb-2 pl-2">Form</th></tr></thead>
              <tbody>
        ...
        <div className="rounded-2xl border border-hairline bg-surface p-5">
          <div className="mb-4 flex items-center justify-between"><h2 className="font-display text-lg uppercase tracking-tight text-primary">Top Scorers</h2><Link to={`/leagues/${LEAGUE_ID}`} className="text-[11px] font-bold uppercase tracking-wider text-brand-text">View all</Link></div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          {QUICK.map((a) => (
            <Link key={a.label} to={a.to} className="group relative rounded-2xl border border-hairline bg-surface p-4 transition-colors hover:border-brand/40">
              {a.badge && <span className="absolute right-3 top-3 flex h-5 min-w-5 items-center justify-center rounded-full bg-red px-1 text-[10px] font-bold text-white">{a.badge}</span>}
