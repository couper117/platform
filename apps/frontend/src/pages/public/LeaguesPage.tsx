    ...
    >
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center space-x-2 text-[10px] uppercase font-bold tracking-widest opacity-40">
          <span>{enumLabel('sport', league.sport?.name)}</span>
          <span>•</span>
          <span>{t('leagues.season', { season: league.season })}</span>
        </div>
      </div>
      ...
      <div className="mt-8 grid grid-cols-2 gap-4 border-t border-surface-3 dark:border-white/5 pt-6">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-xs font-bold opacity-60">{t('leagues.team_count', { count: league._count?.teams || 0 })}</span>
        </div>
        <div className="flex items-center space-x-2">
          <Calendar size={14} className="text-red opacity-60" />
          <span className="text-xs font-bold opacity-60">{enumLabel('gender', league.gender)}</span>
        </div>
      </div>

      <div className="mt-6 flex items-center justify-between group-hover:text-red transition-colors">
        <span className="text-[10px] font-bold uppercase tracking-[0.2em]">{t('leagues.view_standings')}</span>
      ...
      <section className="bg-surface-dark py-20 relative overflow-hidden">
        ...
        <ResponsiveWrapper className="relative z-10 text-center space-y-4">
          <h1 className="text-5xl sm:text-7xl font-display text-white uppercase tracking-tighter">
            {t('leagues.title')} <span className="text-red">{t('leagues.title_accent')}</span>
          </h1>
          <p className="text-white/40 max-w-xl mx-auto uppercase tracking-[0.3em] text-[10px] font-bold">{t('leagues.subtitle')}</p>
        </ResponsiveWrapper>
      </section>
        ...
        <ResponsiveWrapper>
          <div className="flex overflow-x-auto scrollbar-hide py-4 space-x-4 items-center no-wrap">
            <div className="flex-shrink-0 flex items-center space-x-2 px-4 py-2 border-r border-surface-3 dark:border-white/10 mr-2">
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest opacity-40">{t('common.filter')}</span>
            </div>
            ...
            >
              <option value="">{t('leagues.all_sports')}</option>
              {sports?.data?.map(s => <option key={s.id} value={s.id}>{enumLabel('sport', s.name)}</option>)}
            </select>
            ...
            <select
              ...
              aria-label={t('leagues.filter_gender')}
            >
              <option value="">{t('leagues.all_genders')}</option>
            ...
            <select
              ...
              aria-label={t('leagues.filter_level')}
            >
              <option value="">{t('leagues.all_levels')}</option>
      ...
      <ResponsiveWrapper className="mt-12">
        {isLoading ? (
          ...
          <div className="py-40 text-center space-y-6">
            ...
            <div className="space-y-2">
              <h3 className="text-3xl font-display uppercase tracking-tight opacity-40">{t('leagues.none')}</h3>
              <p className="text-sm opacity-30 uppercase tracking-widest">{t('leagues.none_hint')}</p>
            </div>
            <button
