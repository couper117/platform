...
// APR FC (id 1) vs Rayon Sports (id 2) — Rwanda Premier League. See seed.ts.
const FIXTURE_ID = 74;
...
async function seedTeam(teamId: number, formation: string, coachName: string, captainCat: Cat) {
  ...
  await prisma.matchTeamSheet.deleteMany({ where: { fixtureId: FIXTURE_ID, teamId } });
  await prisma.lineup.deleteMany({ where: { fixtureId: FIXTURE_ID, teamId } });

  await prisma.matchTeamSheet.create({
    data: { fixtureId: FIXTURE_ID, teamId, formation, coachName, published: true },
  });
...
async function main() {
  ...
  if (!fixture) throw new Error(`Fixture ${FIXTURE_ID} not found — adjust FIXTURE_ID.`);
