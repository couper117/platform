...
module.exports = {
  ...
  // A rejection has to say why. The club sees reviewNote on their documents page
  // and has no other way to learn what to fix — "REJECTED" on its own sends them
  // back to re-upload the same file.
  reviewDocument: z.object({
    body: z.object({
      status: z.enum(['APPROVED', 'REJECTED']),
      reviewNote: z.string().optional(),
    }).passthrough().refine(
      (b) => b.status !== 'REJECTED' || (b.reviewNote || '').trim().length >= 3,
      { message: 'Say why it was rejected, so the club knows what to correct', path: ['reviewNote'] },
    ),
    query: z.any().optional(),
    params: z.any().optional(),
  }),
