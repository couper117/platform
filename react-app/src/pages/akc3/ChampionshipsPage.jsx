...
const ChampionshipCard = ({ c, t }) => (
  <Card hover className="p-6 relative overflow-hidden">
    <div className="absolute top-0 right-0 w-28 h-28 bg-red/5 -mr-12 -mt-12 rounded-full" />
    <div className="relative z-10 space-y-4">
      <div className="flex items-start justify-between">
        <span className="p-3 bg-red/10 rounded-2xl text-red"><Trophy size={24} /></span>
      ...
      <div className="flex flex-wrap gap-4 text-[10px] font-bold uppercase tracking-widest opacity-50">
        <span className="flex items-center gap-1.5"><Layers size={12} className="text-red" />{c.level || 'National'}</span>
        {c.venue && <span className="flex items-center gap-1.5"><MapPin size={12} className="text-red" />{c.venue}</span>}
        {c.startDate && <span className="flex items-center gap-1.5"><Calendar size={12} className="text-red" />{format(new Date(c.startDate), 'dd MMM yyyy')}</span>}
      </div>
      <div className="flex items-center justify-between pt-4 border-t border-surface-3 dark:border-white/5 text-[10px] uppercase font-bold tracking-widest">
        ...
        <Link to="/amashuri/standings" className="flex items-center gap-1 text-red hover:underline">
